...
#import "NewsHeadScrollView.h"
#import "ContentViewController.h"
#define  CONTENTOFFSET 70
...
- (void)addNews {
    ...
    _addImageView.frame = CGRectMake(maxX(self.headScrollView), 64, 40, 40);
    _addImageView.backgroundColor = [UIColor yellowColor];
...
-(NSArray *)headTitleArray {
    if (!_headTitleArray) {
        _headTitleArray = [NSArray arrayWithObjects:@"头条",@"娱乐",@"热点",@"体育",@"郑州",@"财经",@"科技",@"NBA",@"健康",@"足球", nil];
    }
...
- (void)configHeadScrollView {
    self.headScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH-40, HEADSCROLLVIEW_HEIGHT)];
    self.headScrollView.backgroundColor = [UIColor purpleColor];
    self.headScrollView.contentSize = CGSizeMake(self.headTitleArray.count * 70, 40);
...
- (void)addBtnsToHeadScrollView {
    ...
    for (int i = 0; i < self.headTitleArray.count; i ++) {
        ...
        button.frame = CGRectMake(0 + 70 * i, 0, 70, 40);
...
- (void)scrollHeadScrollView {
    ...
    CGFloat x = self.currentPage * 70 + 70 * 0.5 - SCREEN_WIDTH * 0.5;
    if (x >= 0) {
        if (x >= self.headScrollView.contentSize.width - SCREEN_WIDTH) {
            x = self.headScrollView.contentSize.width - SCREEN_WIDTH;
            [self.headScrollView setContentOffset:CGPointMake(x+20, 0) animated:YES];   //向右滚动到尽头
        }else
